...
async def technical_node(state: InterviewState) -> dict:
    ...
    if state["last_response"]:
        ...
        should_advance = (
            avg_technical >= settings.technical_threshold_deep_dive
            and confidence_trend in ["improving", "stable"]
            and state["questions_in_stage"] >= 3
        )
    ...
    if (
        ...
        or updates.get("struggle_count", 0) >= 3
    ...
    # Determine which skill to focus on based on coverage - handle both dict and LangChain messages
    covered_topics = []
    for msg in state["messages"]:
        if is_assistant_message(msg):
            content = get_message_content(msg)
            covered_topics.append(content)
