...
async def start_interview(request: StartInterviewRequest):
    ...
    try:
        ...
        if not current_question and state.get("messages"):
            current_question = state["messages"][-1].get("content", "")
...
async def submit_response(request: RespondRequest, background_tasks: BackgroundTasks):
    ...
    # Add to messages with correct role
    candidate_message = {
        "role": "human",  # LangChain standard role
        "content": request.response,
        "timestamp": datetime.utcnow().isoformat(),
        "evaluation": None,
    }
    if isinstance(state.get("messages"), list):
        state["messages"].append(candidate_message)
    else:
        state["messages"] = [candidate_message]
    ...
    try:
        ...
        try:
            background_tasks.add_task(
                ...
                confidence_score=state.get("confidence_scores", [0.5])[-1] if state.get("confidence_scores") else 0.5,
                technical_score=state.get("technical_scores", [0.5])[-1] if state.get("technical_scores") else 0.5,
